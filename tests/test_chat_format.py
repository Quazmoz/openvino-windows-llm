...
def test_build_prompt_within_budget_binary_search_exact():
    ...
    prompt_sys, tokens_sys = build_prompt_within_budget(system_only, render_chatml, _count_words, max_prompt_len=2)
    assert "sys" in prompt_sys

    # Verify monotonic dropping and correctness for multiple budget limits
    for max_len in range(5, 50):
        prompt, tokens = build_prompt_within_budget(msgs, render_chatml, _count_words, max_prompt_len=max_len)
        # It must fit, or if even keeping just the newest turn exceeds budget, it keeps that newest turn
        newest_only_len = _count_words(render_chatml([msgs[0], msgs[-1]], add_generation_prompt=True))
