...
SECRET_KEY_RE = re.compile(r"(?:api[_-]?key|authorization|bearer|hf[_-]?token|access[_-]?token|secret|password|certificate|private[_-]?key)", re.IGNORECASE)
SECRET_VALUE_RE = re.compile(r"(?:hf_[A-Za-z0-9_=-]{8,}|Bearer\s+[A-Za-z0-9._~+/=-]+|(?:api[_-]?key|token|secret|password)\s*[:=]\s*[^\s,;]+)", re.IGNORECASE)
EMAIL_RE = re.compile(r"\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b", re.IGNORECASE)
WINDOWS_USER_RE = re.compile(r"(?i)([A-Z]:\\Users\\)[^\\/\s]+")
POSIX_HOME_RE = re.compile(r"(?<![A-Za-z0-9_])/(?:home|Users)/[^/\s]+")
CONTROL_RE = re.compile(r"[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]")
SENSITIVE_CONTENT_LINE_RE = re.compile(r"(?:raw\s+request|request\s+body|prompt(?:_text)?\s*[:=]|chat\s+history|messages?\s*[:=]|source\s+image)", re.IGNORECASE)
...
def benchmark_summary(run: Mapping[str, Any]) -> Mapping[str, Any]:
    result_keys = {
        "model_id", "requested_device", "actual_device", "load_time_ms",
        "time_to_first_token_ms", "total_latency_ms", "completion_tokens",
        "tokens_sec", "success", "error", "timestamp", "runs", "score",
    }
    results = []
    ...
    return {
        key: run.get(key)
        for key in (
            "run_id", "created_at", "finished_at", "max_tokens", "runs_per_combo",
            "mock", "recommendation", "caveat",
        )
        if key in run
    } | {"results": results}
...
def local_hardware_snapshot(models_dir: Path) -> Mapping[str, Any]:
    snapshot: dict[str, Any] = {
        "os": {
            ...
            "version": platform.version(), "edition": windows_edition(),
            "architecture": platform.machine(),
        },
        "cpu": {}, "memory": {}, "disk": {}, "runtime": {},
...
def safe_disk_payload(raw: Mapping[str, Any], models_dir: Path) -> Mapping[str, Any]:
    payload = {key: raw.get(key) for key in ("free_gb", "total_gb", "used_gb", "percent") if key in raw}
    try:
...
def certification_summary(parsed: Any) -> Mapping[str, Any]:
    ...
    device_keys = {"device", "base", "full_name", "driver_version", "architecture", "optimization_capabilities"}
    result_keys = {
        "test", "name", "status", "success", "model_id", "requested_device",
        "actual_device", "load_time_ms", "time_to_first_token_ms", "total_latency_ms",
        "completion_tokens", "tokens_sec", "score", "error",
    }
    summary["devices"] = [
    ...
    for key in ("failures", "warnings"):
        ...
        for item in list(parsed.get(key) or [])[:100]:
            text = str(item)
            values.append("[redacted-content]" if SENSITIVE_CONTENT_LINE_RE.search(text) else sanitize_text(text, limit=1000))
        summary[key] = values
    return summary
