...
Local Windows release generation is the primary workflow. GitHub Actions is not required and no automatic release workflow is enabled.
...
```powershell
Get-FileHash .\OpenVINO-Windows-LLM-0.4.0-windows-x64-installer.exe -Algorithm SHA256
python .\scripts\release_tools.py verify-checksums --path .\OpenVINO-Windows-LLM-0.4.0-checksums.txt
```
...
```powershell
.\scripts\publish_release.ps1 -Version 0.4.0 -Channel stable -DryRun
.\scripts\publish_release.ps1 -Version 0.4.0 -Channel stable
```
